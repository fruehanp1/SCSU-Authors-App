...
from .specs import (CitationFormatSpec, CollectionFacetSpec, FieldSpec, FlatFacetSpec, RelationSpec, ScopeSpec, SortSpec,
                    TreeFacetSpec)
...
class Composer:
    ...
    def init_default_facets(self, exclude=None):
        ...
        if 'facet_collections' not in exclude:
            self.add_facet(
                CollectionFacetSpec(
                    key='facet_collections',
                    filter_key='deptartment_or_office',
                    title=('Department or Office'),
                    weight=100,
                    field_type=ID(stored=True),
                    extractor=extractors.CollectionFacetTreeExtractor(),
                    codec=codecs.CollectionFacetCodec(),
                    missing_label=None,
                    sort_key=['label'],
                    sort_reverse=False,
                    item_view=True,
                    allow_overlap=False,
                    query_class=Prefix,
                    collection_key='94U2PZEV' ##grab from the end of the collection url for department or office
                )
            )
