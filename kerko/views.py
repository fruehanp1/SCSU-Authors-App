...
def home():
    ...
    return render_template(current_app.config['KERKO_TEMPLATE_LANDING'], total_count=total_count_formatted)

#building blocks of automated pulls from scopus. Unable to get working 
#@blueprint.route('/scopus-pull', methods=['GET', 'POST'])
#@except_abort(SearchIndexError, 503)
#def scopuspull():
#    """Scopus Pull"""
#    form = LoginForm()
...
#    s = AffiliationSearch(query, download=False, refresh=30)
#    total = s.get_results_size()

#    if entered_pw == pw:
        ##Pull data from scopus and post directly to zotero

#        return redirect(url_for('kerko.search'))

#    return render_template(current_app.config['KERKO_TEMPLATE_SCOPUSPULL'], form=form, total=total)

@blueprint.route('/bibliography/', methods=['GET', 'POST'])
@except_abort(SearchIndexError, 503)
def search():
...
@blueprint.route('/bibliography/<path:item_id>')
@except_abort(SearchIndexError, 503)
...
@blueprint.route('/bibliography/<path:item_id>/download/<string:attachment_id>/')
@blueprint.route('/bibliography/<path:item_id>/download/<string:attachment_id>/<string:attachment_filename>')
@except_abort(SearchIndexError, 503)
...
@blueprint.route('/bibliography/download/<string:item_id>/')
@blueprint.route('/bibliography/download/<string:item_id>/<string:attachment_filename>')
@except_abort(SearchIndexError, 503)
...
@blueprint.route('/bibliography/<path:item_id>/export/<string:citation_format_key>')
@except_abort(SearchIndexError, 503)
...
@blueprint.route('/bibliography/export/<string:citation_format_key>/')
@except_abort(SearchIndexError, 503)
